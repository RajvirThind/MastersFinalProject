...
from BESSOptimisation.milp.DayAheadMILP_optimiser import BESS_Optimiser #MILP
from BESSOptimisation.environments.dayahead_bess_env import BESSEnv   #RL
from stable_baselines3 import PPO
from stable_baselines3.common.vec_env import DummyVecEnv, VecNormalize

def run_comparison(test_df, battery_params, rl_model_path, vec_norm_path):
    """Runs MILP and PPO on the same data and plots the results together."""
    ...
    # --- Running RL Agent (PPO) ---
    print("Running PPO Agent Inference...")
    ...
    # Wrap the environment to scale inputs exactly as they were during training
    ...
    # FIX: Stop at len(test_df) - 2 to prevent DummyVecEnv from auto-resetting and wiping history
    ...
    ax2.step(rl_results.index, rl_results['Power_DayAhead'], where='post', label='PPO Power (MW)', color='tab:blue', alpha=0.6)
    ...
    ax3.plot(rl_results.index, rl_cum_profit, label='PPO Profit', color='tab:orange', linestyle='--')
    ...
    print(f"PPO Cycles:         {rl_results['Throughput'].sum() / battery_params['capacity']:.2f}")
    print("="*40)
...
if __name__ == "__main__":
    ...
    # Pass the PPO model name and the PPO-specific vec_normalize.pkl file
    run_comparison(test_window, params, "../models/bess_ppo_lookahead_normalized.zip", "../models/vec_normalize_ppo.pkl")