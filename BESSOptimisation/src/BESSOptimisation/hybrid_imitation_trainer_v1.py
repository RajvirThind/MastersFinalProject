...
from dayahead_agent_v1 import BESSEnv
...
def train_imitation_model(env, expert_obs, expert_acts):
    ...
    for epoch in range(EPOCHS):
        ...
        for b_obs, b_act in loader:
            # We want the 'mean' action from the latent actor network
            # This is the decision without the noise used for RL exploration
            pred_actions = model.actor(b_obs)
            
            # calculate the MSE and we need to minimise this
            loss = th.nn.functional.mse_loss(pred_actions, b_act)

            optimizer.zero_grad() #clear the memory, in pytorch gradients accumulate by default, before new calculation starts, needs to be cleared
            loss.backward() #looks at the loss, calculates gradients
            optimizer.step() #updates the model parameters
            epoch_loss += loss.item() #keeps the score
