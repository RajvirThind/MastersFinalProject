...
class MultiMarketBESSEnv(gym.Env):
    ...
    def reset(self, seed=None, options=None):
        ...
        self.prev_direction = 0.0 # 1 for discharge, -1 for charge, 0 for idle
    ...
    def step(self, action):
        ...
        # STEP D: Revenue & Smoothing Penalty
        ...
        # --- REVERSAL PENALTY (Anti-Barcoding) ---
        current_dir = np.sign(net_arb_power)
        reversal_penalty = 0.0
        if self.prev_direction != 0 and current_dir != 0 and self.prev_direction != current_dir:
            reversal_penalty = 20.0 # High penalty to force commitment
        self.prev_direction = current_dir

        reward = revenue - deg_cost - stress_cost - reversal_penalty
        
        # Logging
        self.history.append({
            'SOC': self.soc, 'Net_Profit': reward, 'Arb_Power_Used': np.sum(actual_arb_power),
...
if __name__ == "__main__":
    ...
    battery_params = {'time_interval': 0.5, 'max_power': 10, 'capacity': 20, 'rte': 0.9, 'soc_min_factor': 0.1, 'soc_max_factor': 0.9, 'utilisation_factor': 0.02}
    ...
    model = PPO("MlpPolicy", env, learning_rate=5e-5, ent_coef=0.01, batch_size=256, gamma=0.999, verbose=1, policy_kwargs=dict(net_arch=[512, 512]))
