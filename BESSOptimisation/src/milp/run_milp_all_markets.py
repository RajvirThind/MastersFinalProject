...
from milp_optimiser_all_markets import BESS_Optimiser #MILP
import sys
import os
...
def plot_weekly_soc(dispatch_data_list, week_start_day=0):
    ...
    plt.savefig('milp_outputs/weekly_soc_analysis.png', dpi=300, bbox_inches='tight')
    print("Saved: weekly_soc_analysis.png")
...
def plot_finance(fin_data):
    ...
    plt.savefig('bess_financial_jcurve.png', dpi=300, bbox_inches='tight')
    print("Saved: bess_financial_jcurve.png")
...
def simulate_bess_operation():
    ...
    full_price_df = full_price_df.head(2000)
    ...
    for start_idx in range(0, len(full_price_df), steps_per_day):
        ...
        if results is not None:
            ...
            #Calculate profit breakdowns required by plot_long_term_appraisal
    ...
    if dispatch_data_list:
        ...
        print("Generating weekly Revenue Stacking chart...")
        plot_revenue_stacking(dispatch_data_list, week_start_day=0)
